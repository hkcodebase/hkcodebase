# Hemant Kumar | AWS Certified Solutions Architect 

[![Portfolio](https://img.shields.io/badge/Portfolio-hemantkumar.dev-blue?style=for-the-badge&logo=google-chrome)](https://hemantkumar.dev)

[![Medium](https://img.shields.io/badge/Medium-hkcodeblogs-black?style=for-the-badge&logo=medium)](https://hkcodeblogs.medium.com)

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/hemantk01/)

I am a **Software Engineer III** with over **15 years of experience** specializing in building scalable, high-performance distributed systems on AWS. My expertise spans from architecting banking APIs to implementing robust Infrastructure as Code (IaC) solutions.

---

## 🚀 Professional Highlights

* **Current Role:** Software Engineer III at **JPMorgan Chase & Co.**, focusing on high-performance banking APIs and multi-region AWS ECS/EKS clusters.
* **Cloud Expertise:** AWS Certified Solutions Architect & Developer Associate.
* **Infrastructure:** Expert in **Terraform**, Jenkins, and Spinnaker for automated CI/CD and Blue/Green deployments.
* **Backend:** Extensive background in Java, Spring Boot, and Microservices architecture.


## The Enterprise Stack

| Component | Technology |
| --- | --- |
| **Backend/Core** | Java, Spring Boot, Microservices
| **Cloud Platform** | AWS (ECS/EKS, S3, CloudFront, Route53)
| **IaC & DevOps** | Terraform, CI/CD Pipelines (Blue/Green)
| **Data/ETL** | SQL, NoSQL, Data Modeling, ETL Pipelines


---

## ✍️ Latest Blogs & Writing

I share technical insights on **[hkcodeblogs.medium.com](https://hkcodeblogs.medium.com)**, focusing on:

* Scalable System Design and Microservices.


* AWS Architecture and Performance Optimization.


* DevOps Best Practices and CI/CD Automation.

## Project: [hemantkumar.dev](https://hemantkumar.dev)
My portfolio website hosted at hemantkumar.dev is a live demonstration of my **Cloud & DevOps** skills.
Provisioned using automated infrastructure and deployed in AWS using Github Actions.

Complete Guide: [terraform/README.md](terraform/README.md)

### 📂 Repository Structure
```text
.
├── hemantkumardev/      # Frontend: Source code for hemantkumar.dev
├── terraform/           # Infrastructure: Terraform scripts for AWS provisioning
└── terraform/README.md  # Documentation

```

---

## 📬 Contact & Links

* **Portfolio:** [hemantkumar.dev](https://hemantkumar.dev)
* **LinkedIn:** [Hemant Kumar](https://www.linkedin.com/in/hemantk01/)

---
